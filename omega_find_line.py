...
def choose_list(gamma_avg,avg_delta):
  ...
  length = 15 #minimum length of the line 
  num_of_line=10
  #######################################
  #print(max(gamma_avg))
  l_bin=int(max(abs(gamma_avg))//binsize) # #of the bins for omega_t
  if l_bin <= 1 :
    print("This run does not have growth")
    ...
    for t in range(1,iend-istart+1):
      #print(l_omega)
      bin_num = int(abs(gamma_avg[l_omega])//binsize)  # to put into the right bin
      #print(gamma_avg[l_omega],"!!!!", bin_num)
